...
int casasPreenchidas (int tabuleiro[9][9]) { // Conta quantas casas estão preenchidas
    int preenchidas = 0;
    for (int i = 0; i < 9; i++) {
        for (int j = 0; j < 9; j++) {
            if (tabuleiro[i][j] != ' ') {
...
int main() { // Aqui é onde toda a mágica acontece na sua telinha do terminal.
    ...
    while (jogoAcabou(tabuleiro) != 1) { // Looping de jogadas até completar todas as casas.
        exibirTabuleiro(tabuleiro, fixos);
        jogar(tabuleiro, fixos);
    }
